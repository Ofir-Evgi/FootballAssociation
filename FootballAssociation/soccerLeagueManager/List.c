...
// Output:	TRUE if succeeded
//////////////////////////////////////////////////////////////
BOOL L_delete(NODE* pNode, void(*freeS)(const void*))
{
	NODE* tmp;
	if (!pNode)
		return False;
...
NODE* L_find(NODE* pNode, DATA Value, int(*compare)(const void*, const void*))
{
	...
	while (tmp->next != NULL)
	{
		if (compare(tmp->next->key, Value) >= 0) {
			break;
		}
		tmp = tmp->next;
	}
...
// Output:	TRUE if succeeded
////////////////////////////////////////////////
BOOL L_free(LIST* pList, void(*freeS)(const void*))
{
	NODE* tmp;
	BOOL cont = True;
	if (!pList)
